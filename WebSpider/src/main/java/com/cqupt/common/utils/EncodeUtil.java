...
public class EncodeUtil {
	...
	public ContentPojo getWebPageCharset(byte[] byteArray,
		...
		if (byteArray == null || byteArray.length == 0
				|| byteArray.length <= StaticValueOfRule.url_data_min_byte_length) {// 最后的判断是去掉URL对应的数据内容太短的情况
			...
			if (StringOperatorUtil.isBlank(temp_charset)) {
				temp_charset = StaticValueOfRule.SYSTEM_ENCODING;
			}
		...
		String lastEncoding = StaticValueOfRule.SYSTEM_ENCODING;// 最后的charset返回的编码coding
		...
		try {
			...
			if (charsetByMetadata != null) {// 说明从metadata是得到charset，直接用，不进行后边的计算了
				...
				if (!isFoundCharset) {
					...
					if (regexUtil.isAllChineseChar(oldTitle)) {
						contentPojo.setCharset(StaticValueOfRule.SYSTEM_ENCODING);
						return contentPojo;
					}
					// 解决UTF为charset,但实际为utf-8的情况，同时去除一些非中文字的情况
					if (oldCharset == StaticValueOfRule.SYSTEM_ENCODING) {// 如果得到oldCharset是UTF-8，这种情况即实际和charset不符，则默认再按GBK处理一次，默认只有国内有这种情况
						StringTemp4BodyFront bodyFront = new StringTemp4BodyFront();
						// 为contentPojo设置title
						title = getUrlTitle(StaticValueOfRule.GBK_ENCODING,
								byteArray, bodyFront);
						if (regexUtil.isContainFreqChineseChar(bodyFront
								.getBodyFront())) {
							contentPojo.setCharset(StaticValueOfRule.GBK_ENCODING);
							contentPojo.setTitle(title);
						}
	...
	public boolean setCharset(ContentPojo contentPojo, String lastEncoding,
		...
		while ((line = br.readLine()) != null && (!isFoundBody)) {
			...
			if (temp.contains("<meta") && temp.contains("charset")) {
				...
				if (temp.contains(StaticValueOfRule.SYSTEM_ENCODING)) {
					lastEncoding = StaticValueOfRule.SYSTEM_ENCODING;
				} else if (temp.contains(StaticValueOfRule.GB2312_ENCODING)) {
					lastEncoding = StaticValueOfRule.GB2312_ENCODING;
				} else if (temp.contains(StaticValueOfRule.GBK_ENCODING)) {
					lastEncoding = StaticValueOfRule.GBK_ENCODING;
				} else if (temp.contains(StaticValueOfRule.BIG5_ENCODING)) {
					lastEncoding = StaticValueOfRule.BIG5_ENCODING;
				} else if (temp.contains(StaticValueOfRule.Xili_Window_ENCODING)) {
					lastEncoding = StaticValueOfRule.Xili_Window_ENCODING;
				} else if (temp.contains(StaticValueOfRule.Japan_Shift_ENCODING)) {
					lastEncoding = StaticValueOfRule.Japan_Shift_ENCODING;
				} else if (temp.contains(StaticValueOfRule.Japan_Euc_ENCODING)) {
					lastEncoding = StaticValueOfRule.Japan_Euc_ENCODING;
				}
				isFoundCharset = true;
			} else if (temp.contains("<html") && temp.contains("lang=")) {
				if (temp.contains(StaticValueOfRule.Japan_Lang)) {
					lastEncoding = StaticValueOfRule.Japan_Lang_First;
					isFoundCharset = true;
				}
			}
			sb.append(temp);
	...
	public static String getCharsetByMetadata(String content_type) {
		...
		if (content_type != null) {
			...
			if (content_type.contains("charset")) {
				if (content_type.contains(StaticValueOfRule.SYSTEM_ENCODING)) {
					return StaticValueOfRule.SYSTEM_ENCODING;
				} else if (content_type.contains(StaticValueOfRule.GB2312_ENCODING)) {
					return StaticValueOfRule.GB2312_ENCODING;
				} else if (content_type.contains(StaticValueOfRule.GBK_ENCODING)) {
					return StaticValueOfRule.GBK_ENCODING;
				} else if (content_type.contains(StaticValueOfRule.BIG5_ENCODING)) {
					return StaticValueOfRule.BIG5_ENCODING;
				} else if (content_type
						.contains(StaticValueOfRule.Xili_Window_ENCODING)) {
					return StaticValueOfRule.Xili_Window_ENCODING;
				} else if (content_type
						.contains(StaticValueOfRule.Japan_Shift_ENCODING)) {
					return StaticValueOfRule.Japan_Shift_ENCODING;
				} else if (content_type
						.contains(StaticValueOfRule.Japan_Euc_ENCODING)) {
					return StaticValueOfRule.Japan_Euc_ENCODING;
				}
			}
		}
		return null;
	...
	public static byte[] translateByteArray(byte[] bytes, String charset) {
		try {
			return new String(bytes, charset)
					.getBytes(StaticValueOfRule.SYSTEM_ENCODING);
