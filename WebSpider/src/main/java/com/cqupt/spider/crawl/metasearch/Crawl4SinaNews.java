...
public class Crawl4SinaNews {
	// 日志
	public static MyLogger logger = new MyLogger(Crawl4SinaNews.class);
	/*
	...
	public static void init() {
		...
		itemSelector.add("div#result>div.box-result>div.r-info");

		titleSelector.add("h2>a");

		authorSelector.add("h2>span.fgray_time");

		bodySelector.add("div.r-info>p.content");
...
/*		// 由于360搜索在首页跳转时有暂不可解决问题，故采用直接搜索的方式
		...
		//直接获取sina连接 
	String root_url = StaticValue.sina_news_search_url_format.replace(
				"${query}", query_encode);
							...
							for (String itemBlock : itemList) {
								...
								if (StringOperatorUtil
									...
									if (UrlOperatorUtil.isValidUrl(href)) {
										...
										logger.info("sina新闻搜索时，提取的链接有问题，请检查!");
									}
										...
										if (StringOperatorUtil
											...
											if (arr.length == 3) {
												crawlData4PortalSite
														.setAuthor(arr[0]);
												crawlData4PortalSite
														.setPublish_time_string(arr[1]+" "+arr[2]);
									...
									List<String> bodyList = JsoupHtmlParser
											.getNodeContentBySelector(
													...
													DataFormatStatus.CleanTxt,
													false);
									if (StringOperatorUtil.isNotBlankCollection(bodyList)) {
