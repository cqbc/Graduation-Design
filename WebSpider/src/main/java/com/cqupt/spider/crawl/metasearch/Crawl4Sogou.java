...
public class Crawl4Sogou {
	...
	public static void init() {
		...
		itemSelector.add("div.results>div.vrwrap");

		titleSelector.add("div>h3.vrTitle>a");

		authorSelector.add("div>div.news-detail>div.news-info>p.news-from");

		bodySelector.add("div>div.news-detail>div.news-info>p.news-txt>span");
							...
							for (String itemBlock : itemList) {
								...
								if (StringOperatorUtil
									...
									if (UrlOperatorUtil.isValidUrl(href)) {
										...
										logger.info("搜狗元搜索时，提取的链接有问题，请检查!");
									}
