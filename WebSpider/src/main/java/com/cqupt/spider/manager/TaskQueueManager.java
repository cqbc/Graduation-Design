...
public class TaskQueueManager {
	...
	public static MD5OperatorUtil md5Util = new MD5OperatorUtil();
	...
	public static RetStatus addTask(CrawlTaskPojo taskPojo,
			TaskQueueType TaskQueueTypeEnum) {
		if (TaskQueueTypeEnum == TaskQueueTypeEnum.To_Visit) {
		...
		} else if (TaskQueueTypeEnum == TaskQueueTypeEnum.Visiting) {
		...
		} else if (TaskQueueTypeEnum == TaskQueueTypeEnum.Visited) {
		...
		} else if (TaskQueueTypeEnum == TaskQueueTypeEnum.Error_Visit) {
		...
		} else if (TaskQueueTypeEnum == TaskQueueTypeEnum.Circle_Visit) {
			...
			try {
				if (!circleVisitUrls.contains(taskPojo)) {
					...
					TaskTypeEnum taskEnumType=taskPojo.getType();
	...
	public static CrawlTaskPojo getTask(TaskQueueType TaskQueueTypeEnum) {
		if (TaskQueueTypeEnum == TaskQueueTypeEnum.To_Visit) {
			lock_toVisitUrls.lock();
			try {
	...
	public static List<CrawlTaskPojo> getAllToDoCicleTaskList(
			TaskQueueType TaskQueueTypeEnum) {
		if (TaskQueueTypeEnum == TaskQueueTypeEnum.Circle_Visit) {
			lock_circleVisitUrls.lock();
			List<CrawlTaskPojo> list = null;
			try {
			...
			if (taskType == TaskQueueType.Circle_Visit) {
				...
				if (is_judge_in_bloom) {
					...
					if (( !BloomFilterManager
						...
						if (TaskTypeEnum.Url==to_do_task_pojo.getType() &&BloomFilterManager
								.containsTaskInToVisiteBloom(to_do_task_pojo
										.getValueFormat())) {
							
						 if (((matchCrawlPojo = SystemControlerManager.extractorContentManager4SimpleCrawler
